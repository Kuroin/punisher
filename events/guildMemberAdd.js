...
//Sayaç
  if (db.has(`sayac_${member.guild.id}`) === true) {
    ...
    member.guild.channels.cache.get(channel).send(`**${member.user.tag}** Sunucuya katıldı! \`${db.fetch(`sayac_${member.guild.id}`)}\` üye olmamıza son \`${db.fetch(`sayac_${member.guild.id}`) - member.guild.members.cache.size}\` üye kaldı!`)
    }};
  
  //Otorol
  if (db.has(`otoR_${member.guild.id}`) === true) {
  var rol = member.guild.roles.cache.get(db.fetch(`otoR_${member.guild.id}`));
  var rolD = `${member.guild.roles.cache.get(db.fetch(`otoR_${member.guild.id}`)) ? "var" : "yok"}`;
  
  var kanalD = `${member.guild.channels.cache.get(db.fetch(`otoRK_${member.guild.id}`)) ? "var" : "yok"}`;
  ...
  member.roles.add(rol)
    ...
    if (db.has(`otoRK_${member.guild.id}`) === true) {
      if(kanalD === "var"){
    member.guild.channels.cache.get(db.fetch(`otoRK_${member.guild.id}`)).send(`**${member.user.tag}** adlı kullanıcıya başarıyla otomatik rol olarak ayarlanmış olan **${rol.name}** adlı rol verildi!`)
  }}}};
